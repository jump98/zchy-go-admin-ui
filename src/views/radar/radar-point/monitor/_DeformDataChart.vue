...
import { getRadarPointDeformData } from "@/api/radar/radar-point";
...
export default {
  ...
  methods: {
    ...
    async getRadarPointDeformList() {
      ...
      // 追加查询（当前最新时间数据）
      if (this.maxDate && this.dataType == 2) {
    ...
    initChart() {
      ...
      const option = {
        ...
        title: {
          text: `形变数据 - ${this.radarPointInfo.pointName}`,
          left: "center"
        },
        xAxis: {
          type: "time",
    ...
    onLegendselectchanged(params) {
      ...
      if (params.name === "B") {
        this.myChart.setOption({
          ...
          title: {
            text: `距离数据 - ${this.radarPointInfo.pointName}`,
            left: "center"
          }
        });
      }
    ...
    appendChartData(data = []) {
      ...
      const newSeriesAData = data.map(item => [new Date(item.time), item.deformation / 100]);
      const newSeriesBData = data.map(item => [new Date(item.time), item.distance / 100]);
      ...
      this.myChart?.setOption(
        {
          xAxis: {
            ...
            min: new Date(this.seriesAData[0][0]), // 最早一条
