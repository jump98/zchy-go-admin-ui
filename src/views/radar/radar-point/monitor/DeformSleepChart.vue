...
export default {
  ...
  methods: {
    ...
    appendChartData(data = []) {
      ...
      const newSeriesAData = data.map(item => [new Date(item.SvrTime), item.Deformation / 100]);
      const newSeriesBData = data.map(item => [new Date(item.SvrTime), item.Distance / 100]);
