  ...
  <div>
    ...
    <div v-show="radarList.length>0" style="position: absolute; top: 94px; right: 0px; z-index: 10; background: rgba(0, 0, 0, 0.5); padding: 10px; border-radius: 5px; max-height: 80vh; overflow-y: auto; color: white;">
      ...
      <div>
        ...
        <ul style="list-style: none; padding: 0; margin: 0;">
          <li v-for="radar in radarList" :key="radar.radarId" 
              @click="selectRadar(radar)" 
              :style="getRadarItemStyle(radar)">
            {{ radar.radarName }}
          </li>
        </ul>
      </div>
      ...
      <div v-if="selectedRadar" style="margin-top: 10px;">
        ...
        <ul v-if="filteredPoints.length > 0" style="list-style: none; padding: 0; margin: 0;">
          <li v-for="point in filteredPoints" :key="point.id" 
              ...
              :style="{ cursor: 'pointer', padding: '5px', backgroundColor: selectedPoint && selectedPoint.id === point.id ? 'rgba(255, 255, 255, 0.3)' : 'transparent', color: 'white' }">
            {{ point.pointName }}
          </li>
        </ul>
        <div v-else style="padding: 5px; color: white;">
          该雷达没有监测点信息
        </div>
      </div>
    </div>
    ...
    <div class="deformationContainer" style="position:absolute;bottom: 0;right: 0;width: 100%;z-index: 8;">
      <DeformationLineChart
        ...
        :radarName="currentRadarName"
        v-show="timerIdForDeformationData!=null"
      ></DeformationLineChart>
    ...
    <div id="map-container" class="map" ref="cesiumContainer" style="width: 100%; ">
      ...
      <div v-show="timerIdForDeformationData==null" class="mouse-position" style="position: absolute; bottom: 10px; left: 10px; background: rgba(0, 0, 0, 0.5); color: white; padding: 5px 10px; border-radius: 3px; font-size: 12px; z-index: 100;">
        经度: {{ mousePosition.longitude.toFixed(6) }} 纬度: {{ mousePosition.latitude.toFixed(6) }}
      </div>
    </div>
    <!-- 雷达告警对话框 -->
    <RadarAlarmDialog 
      :visible.sync="showAlarmDialog" 
      :radarInfo="selectedRadarForAlarm"
    />
  </div>
</template>
...
import {listDeptRadarPoint,listDeptRadarPointDeformationData,getRadarPointDeformData} from '@/api/admin/radar-point'
import {listSysRadar,getRadarsAlarms} from '@/api/admin/sys-radar'
  ...
  methods: {
    ...
    startRadarTimer() {
        ...
        this.timerIdForRadar = setTimeout(() => {
          ...
          listSysRadar(this.addDateRange({
                    pageIndex: 1,
                    pageSize: 1000,
                    radarId:undefined,
                    radarName:undefined,
    ...
    updateRadars(radars) {
        ...
        if (this.viewer && this.viewer.entities) {
          // 过滤出雷达点实体并移除
          const radarEntities = this.viewer.entities.values.filter(entity =>
            entity.id && entity.id.startsWith(radarPrefix)
          );
    ...
    updateRadarPoints(points){
        ...
        const pointEntities = this.viewer.entities.values.filter(entity =>
          entity.id && entity.id.startsWith(radarPointPrefix)
        );
      ...
      selectPoint(point) {
        ...
        this.startDeformationDataTimer({radarId: point.radarId, index: point.pointIndex});
      },
    init() {
      this.loading=false
    ...
    console.log('imageLayers=',imageryLayers)
